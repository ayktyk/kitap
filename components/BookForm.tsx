...
const BookForm: React.FC<Props> = ({ initialData, onSave, onCancel }) => {
  ...
  return (
    <div className="glass rounded-3xl max-w-4xl mx-auto overflow-hidden flex flex-col h-full md:h-auto border border-white/5 shadow-2xl animate-fade-in-up">
      {/* Header */}
      <div className="bg-white/5 border-b border-white/5 p-6 flex justify-between items-center backdrop-blur-3xl">
        <div>
          <h2 className="text-2xl font-serif font-bold text-white">{initialData ? 'Kitabı Düzenle' : 'Yeni Kitap Ekle'}</h2>
          <p className="text-white/40 text-sm mt-1">Kütüphanenizi zenginleştirin</p>
        </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <p className="text-[10px] uppercase tracking-wider font-bold text-white/20">AI ile otomatik doldurma aktif</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="WANT_TO_READ" className="bg-zinc-900">Okumak İstiyorum</option>
                  <option value="READING" className="bg-zinc-900">Okuyorum</option>
                  <option value="READ" className="bg-zinc-900">Okundu</option>
                  <option value="ABANDONED" className="bg-zinc-900">Yarım Bıraktım</option>
                </select>
              </div>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-white/60">Puanım</label>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 p-3 bg-white/5 text-white border border-white/10 rounded-xl focus:ring-2 focus:ring-white/20 focus:border-transparent outline-none placeholder:text-white/20 transition-all"
      ...
      <div className="p-8 border-t border-white/5 bg-white/[0.02] flex justify-end gap-4">
        <button
          onClick={onCancel}
          className="px-8 py-3 rounded-xl border border-white/10 text-white/60 font-bold text-xs uppercase tracking-widest hover:bg-white/5 transition-all"
        >
          İptal
        </button>
        <button
          onClick={() => onSave(formData)}
          className="px-8 py-3 rounded-xl bg-white text-black font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] flex items-center gap-2"
        >
          <Save size={16} />
          Kaydet
        </button>
